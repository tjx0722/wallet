	...
	<div id="app">
		<TABLE border="1" width="100%">
			...
			<TR align="center" v-for="user in json.list">
				...
				<TD>
					...
					<button
						@click="!user.isdelete?deleted(user.userid):revoke(user.userid)"
						class="btn">{{!user.isdelete?'删除':'恢复'}}</button>
				</TD>
				<TD>
		...
		<TABLE border="1" width="100%">
			<tr align="center">
				...
				<td width="12.5%">第<input type="text" id="pageNow"
					name="pageNow" v-model="pageNow" size="1">页
				</td>
				<td width="12.5%">一页 <input type="text" name="limit"
					v-model="limit" id="up" size="1">行
				</td>
				<td width="12.5%">{{json.page.pageNow }}/{{json.page.pageCount
					}}</td>
		...
		var vm = new Vue(
				{
					...
					methods : {
						...
						next : function() {
							this.pageNow = parseInt(this.pageNow) == this.json.page.pageCount ? this.json.page.pageCount
									: parseInt(this.pageNow) + 1;
							this.get();
						},
						before : function() {
							this.pageNow = parseInt(this.pageNow) == 1 ? 1
									: parseInt(this.pageNow) - 1;
							this.get();
						},
						end : function() {
						...
						goupdate : function(userid) {
							location.href = "/authorityModule/goupdate/"
									+ userid;
						}
